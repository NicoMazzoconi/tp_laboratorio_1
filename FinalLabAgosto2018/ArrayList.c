...
/** \brief Recibe un arraylist y una funcion, y compara cuantos elementos del arraylist son validos para la funcion
 *
 * \param ArrayList* this elemento a comprar
 * \param int *pFunc la funcion que compara el elemento
 * \return ArrayList* devuelve un nuevo arraylist cargado con los elementos que pasaron por la funcion
 *
 */
ArrayList* al_filter(ArrayList* this, int (*pFunc)(void*))
{
    ArrayList* pListFilter = NULL;
    if(this != NULL && pFunc != NULL)
    {
        if(al_len(this) > 0)
        {
            pListFilter = al_newArrayList();
            int i;
            for(i = 0; i < al_len(this); i++)
            {
                if(pFunc(this->pElements[i]) == 1)
                {
                    al_add(pListFilter, this->pElements[i]);
                }
            }
        }
    }
